...
#,### 패턴에서 자바가 가장 중요하게 보는 건 쉼표가 뒤에서부터 몇 번째 자리에 찍혀 있느냐다.  
#,###나 ###,###이나 똑같이 뒤에서 3번째에 쉼표가 찍히는 규칙으로만 해석된다.  
...
12.5 → 12.5  
12 → 12 (소수점 자체가 사라짐)  
문제점: 0.5 같은 소수를 출력하면 .5가 나온다. 이를 해결하려면 0.###패턴을 사용해야 한다.
